...
# Output directories
os.makedirs('Ref-policy/images', exist_ok=True)
...
def save_img(img_array, img_name):
    ...
    img.save(os.path.join('Ref-policy/images', img_name))
...
def main():
    ...
    for _ in range(num_p_control):
        ...
        theta_dot_init = np.random.uniform(-0.15, 0.15)
    ...
    print(f"Dataset generation complete: {len(data)} images ({num_random} random, {num_p_control} P controller).")
    env.close()
